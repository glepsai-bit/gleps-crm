...
## 🧪 Em QA
- [T-014] **Whitelabel Gleps IA (deploy clone, stack 100% separada)** — @qa — branch: `whitelabel/gleps-ia` (5 commits ahead de origin/main, sem push) — Rebrand UI + backend + infra completo. Detalhes/criticas/pendencias no handoff [2026-06-15 whitelabel gleps ia QA](handoff-log.md#2026-06-15-whitelabel-gleps-ia-qa). **Pendencias pre-merge (Critic UI REFUTOU):** (1) `.logo-glow` em `src/index.css:759,762` ainda vermelho MyChooice; (2) 7 ocorrencias de `alt="MyChooice"` + import `mychooiceLogo` nos 3 layouts/LoginPage; (3) `EmailPreviewDialog.tsx:37` link `#EE3924` no template inline. Critic Infra APROVOU (isolamento ZERO-SHARING). Workdir sujo: package-lock + 2 favicons. Slogan oficial: "A inteligência comercial que seu negócio precisa."
