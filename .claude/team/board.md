...
- [T-014] **Whitelabel Gleps IA (deploy clone, stack 100% separada)** — @frontend (UI) + @dev-principal (backend strings + infra) — branch: `whitelabel/gleps-ia` — Mesmo código do GoodLeads, mas com marca **Gleps IA** (roxo). Cliente novo, deploy independente em `crm.gleps.com.br` (mesma VPS, novo service no EasyPanel, **backend próprio + banco próprio + redis próprio**). Detalhes técnicos no doc [WHITELABEL_GLEPS_IA.md](../../WHITELABEL_GLEPS_IA.md) (raiz do worktree, fonte única) e no handoff [2026-06-15 whitelabel gleps ia](handoff-log.md#2026-06-15-whitelabel-gleps-ia). **Bloqueio único:** logo PNG do cliente precisa ser salvo em `public/favicon.png` (substituir o atual) — usuário vai me passar o arquivo no filesystem. Slogan oficial: "A inteligência comercial que seu negócio precisa."
